...
@app.route('/stats')
def stats():
    url = "https://z94gc1lts1.execute-api.us-east-1.amazonaws.com/plasma/bloodgroupcount"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://z94gc1lts1.execute-api.us-east-1.amazonaws.com/plasma/requestblooduser?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    emailids=[]
    for i in a:
